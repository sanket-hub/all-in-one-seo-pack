...
const makeCancelable = (promise) => {
	...
	const wrappedPromise = new Promise((resolve, reject) => {
		promise.then(
			(val) =>
				hasCanceled_ ? reject({ isCanceled: true }) : resolve(val), /* eslint prefer-promise-reject-errors: "off" */
			(error) =>
...
function LinkControl ({
		...
		return Promise.resolve([
			{
				id    : val,
				title : val,
				url   : 'URL' === type ? prependHTTP(val) : val,
				type
			}
		])
	}
	 ...
	 * @returns {boolean}   whether or not the value is potentially a URL.
	 */
