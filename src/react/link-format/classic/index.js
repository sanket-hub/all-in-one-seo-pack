...
(function ($, aioseoL10n, wp) {
	...
	window.wpLink = {
		...
		mceRefresh: function (searchStr, text) {
			...
			if (linkNode.length) {
				...
				if (href !== '_wp_link_placeholder') {
					...
					inputs.submit.val(aioseoL10n.update);
				} else {
		...
		htmlUpdate: function () {
			...
			wp.a11y.speak(aioseoL10n.linkInserted);
		},
		...
		setDefaultValues: function (selection) {
			...
			// Update save prompt.
			inputs.submit.val(aioseoL10n.save);
		},

		searchInternalLinks: function () {
			var waiting,
				search = inputs.search.val() || '',
				minInputLength = parseInt(aioseoL10n.minInputLength, 10) || 3;
			...
			if (!results) {
				if (firstPage) {
					list += '<li class="unselectable no-matches-found"><span class="item-title"><em>' +
						aioseoL10n.noMatchesFound + '</em></span></li>';
				}
			} else {
				$.each(results, function () {
					...
					list += this.title ? this.title : aioseoL10n.noTitle;
