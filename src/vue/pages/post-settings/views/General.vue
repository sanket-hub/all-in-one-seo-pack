...
export default {
	...
	data () {
		return {
			...
			strings           : {
				...
				keyphraseDocumentation : this.$t.sprintf(this.$t.__('Not sure what keyphrases are used for? Check out our documentation for more information. %1$s', this.$td), this.$links.getDocLink(this.$constants.GLOBAL_STRINGS.learnMore, 'useKeyphrasesTooltip', true))
			}
		}
	},
	watch : {
	...
	computed : {
		...
		title () {
			...
			return this.$t.sprintf(this.$t.__('%1$s Title', this.$td), this.currentPost.type)
		},
		toggled : function () {
	...
	methods : {
		...
		setContent () {
			const debounced = () => debounce(() => {
				const coreEditor      = window.wp.data ? window.wp.data.select('core/editor') : null
				this.setIsDirty()
				if ('post' === this.currentPost.context && 'attachment' !== this.currentPost.postType && 'product' !== this.currentPost.postType && coreEditor) {
					this.runAnalysis({ postId: this.currentPost.id, postData: this.currentPost, content: coreEditor.getEditedPostContent(), slug: coreEditor.getPermalink() })
				}
			}, 1000)
			debounced()
		},
