...
function numberOfSubheadingsReflectingTopic (keyphrase, subheadings) {
	return subheadings.filter(subheading => {
		const keywordMatched = wordMatch(stripTags(subheading), keyphrase, window.aioseo.locale || 'en_US')
...
function keyphraseInSubHeadings (content, keyphrase) {
	...
	if (0 !== subheadings.length) {
		...
		result.matches = numberOfSubheadingsReflectingTopic(keyphrase, subheadings)
