...
function titleLength (content) {
	...
	const titleLength = count(content, 'characters_including_spaces')
	if (0 === titleLength) {
		return {
			title       : __('SEO Title length', td),
	...
	if (titleLength <= recommendedMaximumLength) {
		return {
			title       : __('SEO Title length', td),
	...
	if (titleLength >= recommendedMaximumLength && titleLength <= maximumLength) {
		return {
