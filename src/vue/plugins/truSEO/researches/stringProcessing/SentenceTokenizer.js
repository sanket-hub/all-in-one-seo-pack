...
export default class SentenceTokenizer {
	...
	isSentenceEnding (token) {
		return (
			!isUndefined(token) &&
			('full-stop' === token.type || 'sentence-delimiter' === token.type)
		)
	}
	...
	getSentencesFromTokens (tokenArray, trimSentences = true) {
		...
		do {
			...
			if (firstToken && lastToken && 'html-start' === firstToken.type && 'html-end' === lastToken.type) {
				tokenArray = tokenArray.slice(1, tokenArray.length - 1)

				sliced = true
			}
		} while (sliced && 1 < tokenArray.length)
		...
		tokenArray.forEach((token, i) => {
			...
			switch (token.type) {
				...
				case 'full-stop':
					...
					hasNextSentence = 2 <= nextCharacters.length
