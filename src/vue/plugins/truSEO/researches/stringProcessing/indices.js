...
import { isWordInSentence as matchWordInSentence, characterInBoundary } from '../stringProcessing/matchWordInSentence.js'
...
function getIndicesByWord (word, text) {
	var startIndex = 0,
	 searchStringLength = word.length,
	 index, indices = []
	while (-1 < (index = text.indexOf(word, startIndex))) {
		// Check if the previous and next character are word boundaries to determine if a complete word was detected
		var isPreviousCharacterWordBoundary = characterInBoundary(text[index - 1]) || 0 === index,

		 isNextCharacterWordBoundary = characterInBoundary(text[index + searchStringLength]) || (text.length === index + searchStringLength)
 ...
 */
	filterIndices = function (indices) {
 ...
 * Returns an empty array if none are found.
 */
	getIndicesByWordListSorted = function (words, text) {
		var matchedWords = []
